    ...
    for horizon_hours in _HORIZONS_HOURS:
        for window_days in _WINDOW_DAYS:
            for symbol in [None, *symbols]:
                rows = get_outcomes_for_metrics(horizon_hours, window_days, symbol)
                metrics = _compute_metrics(rows)
                if metrics is None:
                    continue

                insert_signal_metrics(symbol=symbol, horizon_hours=horizon_hours, window_days=window_days, **metrics)

                logger.info(
                    f"symbol={symbol or 'ALL'} horizon={horizon_hours}h window={window_days}d "
